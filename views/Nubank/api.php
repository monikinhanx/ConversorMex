...
<body style="<?= $bg ?>">
    <main class="d-flex justify-content-end">
        ...
        <section class="box-interno my-auto">
            ...
            <div class="container">
                <h1 class="mb-5 title">Envio para API</h1>            
                <p class="sub mb-3">Foram enviados <?=$_SESSION["qtdRes"]?> interações de Chat com sucesso.</p>
                <p class="sub mb-3">Tivemos <?=$_SESSION["qtdErro"]?> erros no envio para API da CallMiner</p>
                <textarea class="form-control" name="resultado" id="resultado" hidden><?=json_encode($_SESSION["resultado"]) ?></textarea>
                <textarea class="form-control" name="erros" id="erros" hidden><?=json_encode($_SESSION["erro"]) ?></textarea>
                
                <a href="/?login" class="btn btn-mex mb-5">Voltar</a>
            </div>
        </section>
    </main>
