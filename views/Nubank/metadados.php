...
<body style="<?= $bg ?>">
    <main class="d-flex justify-content-end">
        ...
        <section class="box-interno my-auto">
            ...
            <div class="container">
                <h1 class="mb-5 title">Quantidade de Arquivos</h1>            
                <p class="sub mb-3">Você está enviando <?=$_SESSION["qtdRes"] ?> interação(ões) de Chat.</p>
                <p class="sub mb-3">Tivemos <?=$_SESSION["qtdErro"] ?> erro(s) nas interações de Chat.</p>
                <p class="sub mb-3">Confirma o upload dessas interações para a API da CallMiner?</p>
                <form action="/?api" method="POST">
                    <div class="form-group">
                        <textarea class="form-control" name="metadados" id="metadados" hidden><?=json_encode($_SESSION["resultado"]) ?></textarea>
                    </div>
                    <div class="form-group">
                        <textarea class="form-control" name="erros" id="erros" hidden><?=json_encode($_SESSION["erro"]) ?></textarea>
                    </div>
                    <a href="/?nubank" class="btn btn-mex mb-5">Voltar</a>
                    <button type="submit" class="btn btn-mex mb-5">Confirmo</button>
                </form>
            </div>
        </section>
    </main>
